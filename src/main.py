...
def run_experiment(k_values, combine_methods, selected_dataset_index, model_name, window_size):
	...
	for k in k_values:
		for combine_method in combine_methods:
			weighted_scores = compute_weighted_scores(window_pred_probabilities, scores, combine_method, k)
			
			# Compute the metric value of the combined anomaly score
			metric_results = compute_metrics(raw_anomalies, weighted_scores)
			results_df = pd.DataFrame(metric_results, timeseries_names, columns=[metric])

			# Save the results dictionary to a CSV file
			results_df.to_csv(os.path.join("reports", "results_04_2024", f"{datasets[selected_dataset_index]}_{model_name}{window_size}_{combine_method}_k{k}.csv"))
